...
class FilesTab(ttk.Frame):
    ...
    def _get_dav_root(self) -> str:
        if not self._dav_root:
            raw = self.settings_service.get_setting("dav_root", "./dav_root")
            self._dav_root = os.path.abspath(raw)
    ...
    def refresh(self) -> None:
        self._dav_root = ""
        root = self._get_dav_root()
        self.root_path_label.config(text=root)
    ...
    def _list_dir(self, dir_path: str) -> None:
        ...
        rel = os.path.relpath(dir_path, self._get_dav_root())
        display = "/" if rel == "." else "/" + rel.replace("\\", "/")
        self.path_label.config(text=display)
        self.up_btn.config(state=tk.NORMAL if dir_path != self._get_dav_root() else tk.DISABLED)

    def _display_entries(self, entries: list[dict]) -> None:
        self.tree.delete(*self.tree.get_children())
        for e in sorted(
            entries,
            key=lambda f: (0 if f["is_directory"] else 1, f["name"].lower())
        ):
            ...
            modified_str = e["modified"].strftime("%Y-%m-%d %H:%M") if isinstance(e["modified"], datetime) else str(e["modified"])
    ...
    def go_up(self) -> None:
        ...
        root = self._get_dav_root()
        if parent and os.path.commonpath([parent, root]) == root:
        ...
        def run():
            for src in paths:
                try:
                    ...
                    logger.info(f"上传文件: {src} -> {dst}")
                except Exception as e:
                    self.after(0, lambda s=src: messagebox.showerror("上传失败", f"{s}: {e}", parent=self))
                    return
            self.after(0, self._list_dir, self._current_path)
    ...
    def download_selected(self) -> None:
        ...
        def run():
            for item in items:
                ...
                name = values[0]
                ftype = values[1]
                src = os.path.join(self._current_path, name)
    ...
    def rename_file(self) -> None:
        ...
        if not new_name or new_name == old_name:
            return
        src = os.path.join(self._current_path, old_name)
        dst = os.path.join(self._current_path, new_name)
        try:
            os.rename(src, dst)
