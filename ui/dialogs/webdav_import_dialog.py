...
class WebDAVImportDialog(tk.Toplevel):
    ...
    def _create_advanced_settings(self, parent):
        ...
        ttk.Label(conn_f, text='块大小(KB):').grid(row=1, column=2, sticky="w", padx=5)
        self.chunk_size_entry = ttk.Entry(conn_f, width=10); self.chunk_size_entry.insert(0, '64'); self.chunk_size_entry.grid(row=1, column=3, pady=2)
        ...
        ttk.Label(proxy_f, text='用户名/密码:').grid(row=1, column=0, sticky="w", padx=5)
        self.proxy_user_entry = ttk.Entry(proxy_f, width=12); self.proxy_user_entry.grid(row=1, column=1, pady=2)
        self.proxy_pass_entry = ttk.Entry(proxy_f, width=12, show='*'); self.proxy_pass_entry.grid(row=1, column=2, pady=2)
    ...
    def _import_task(self, logic, progress_win):
        try:
            ...
            for i, filename in enumerate(target_files):
                ...
                if logic.download_file(f"{logic.options['webdav_hostname']}{filename}", temp_file):
                    with open(temp_file, 'r', encoding='utf-8') as tf:
                        # 调用回调并获取详细操作类型
                        res_uid, op_type = self.on_import_callback(tf.read())
                        if res_uid:
                            success_count += 1
            ...
            progress_win.update_progress(100); progress_win.update_status(f"完成! 成功导入 {success_count} 个文件")
            progress_win.set_finished()
            # 在任务最后可以显式调用一次刷新（如果需要）
        except Exception as e: progress_win.log(f"发生错误: {str(e)}"); progress_win.set_finished()
